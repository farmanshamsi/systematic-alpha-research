"""Build the frozen Day 24 CQF technical-report evidence payload."""
...
GENERATED_AT: Final[str] = "2026-08-10T00:00:00Z"
ARTIFACT_VERSION: Final[str] = (
    "day24_technical_report_v1_10_visible_text_and_notice_layout"
)
...
REPORT_MATH_GROUPS: Final[tuple[tuple[str, int, tuple[str, ...]], ...]] = (
    ...
    (
        "ou_ar1",
        1,
        (r"x_t=a+\phi x_{t-1}+\varepsilon_t",),
    ),
    (
        "ou_diagnostics",
        2,
        (
            r"\mu=\frac{a}{1-\phi}",
            r"\sigma_x=\frac{\sigma_\varepsilon}{\sqrt{1-\phi^2}}",
            r"h_{1/2}=-\frac{\log 2}{\log\phi}",
            r"z_t=\frac{x_t-\mu}{\sigma_x}",
            r"\mathrm{VR}_t(q)=\frac{\mathrm{Var}(x_t-x_{t-q})}"
            r"{q\,\mathrm{Var}(x_t-x_{t-1})}",
        ),
    ),
    (
...
The log ratio is dimensionless and centers the close relative to recent traded activity. It is not assumed stationary by construction. Stationarity is treated as a rolling compatibility question. Over `n_OU` transitions, ordinary least squares estimates

        [[AXIOM_LATEX_OU_AR1]]

An OU-compatible window requires `0 < phi < 1`. For compatible estimates:

        [[AXIOM_LATEX_OU_DIAGNOSTICS]]

The continuous-time OU process is the classical mean-reverting Gaussian process associated with Uhlenbeck and Ornstein [7]. Here it is used only as a diagnostic mapping from a rolling AR(1), not as a claim that the residual follows a globally stable diffusion. The variance ratio follows the idea of comparing multi-period with one-period variance [4]. A value below one is consistent with negative serial dependence, but even the original variance-ratio literature cautions that rejecting a random walk does not prove a profitable mean-reversion strategy.
